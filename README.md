...
`tools/list` at `https://gateway.pipeworx.io/onthisday/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/onthisday_get_events \
  -H 'Content-Type: application/json' \
  -d '{"month":7,"day":20}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/onthisday_get_events`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
